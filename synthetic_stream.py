    ...
    def generate(self, total_length: int, drift_specs: list) -> SyntheticStream:
        ...
        for seg_idx, (seg_start, seg_end) in enumerate(zip(starts, boundaries)):
            ...
            for i in range(seg_start, seg_end):
                if seg_idx > 0 and drift_specs[seg_idx - 1].drift_type == "gradual":
                    spec = drift_specs[seg_idx - 1]
                    tw = spec.transition_width
                    progress = min((i - seg_start) / max(tw, 1), 1.0)
                    m = spec.mean_before + progress * (spec.mean_after - spec.mean_before)
                    s = spec.std_before  + progress * (spec.std_after  - spec.std_before)
...
def plot_stream(stream: SyntheticStream, window: int = 50,
                title: str = "Synthetic data stream") -> plt.Figure:
    """
    ...
    roll_std  = np.array([
        data[max(0, i - window // 2): i + window // 2].std()
        for i in range(N)
    ])
    ...
    ax.plot(roll_mean, color="#534AB7", linewidth=1.2, label=f"rolling mean (w={window})")
    ax.set_ylabel("Value")
    ...
    for spec, col in zip(stream.drift_specs, DRIFT_COLORS):
        for ax in axes:
            ax.axvline(spec.position, color=col, linestyle="--",
...
def plot_detection_report(
    ...
    for alarm in sorted(alarm_positions):
        ...
        candidates = [
            (i, p) for i, p in enumerate(true_positions)
    ...
    ax1.axhline(threshold, color="#BA7517", linestyle="-.", linewidth=1.2,
                label=f"Threshold = {threshold}")

    # True drift lines
    for i, (spec, col) in enumerate(zip(stream.drift_specs, DRIFT_COLORS)):
        ax1.axvline(spec.position, color=col, linestyle="--",
                    linewidth=1.5, label=f"True drift {i+1} (t={spec.position})")

    # Alarm lines
    for alarm in sorted(alarm_positions):
        ax1.axvline(alarm, color="#1D9E75", linestyle=":",
